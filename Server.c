...
int main(int argc, char* argv[])
{
    ...
    while ((opt = getopt(argc, argv, "hp:")) != -1)
    {
        switch (opt)
        {
            ...
            // -p port
            case 'p':
                port = atoi(optarg); // optarg?
                break;
        }
    }

    // ensure port is a non-negative short and path to server's root(arv[optind]) is specified
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            const char* haystack = request; //buffer for request
            ...
            else if (needle - haystack + 2 > LimitRequestLine) // 8190
            {
                error(414);
                continue;
            }   
            char line[needle - haystack + 2 + 1]; // space for CRLF and null teminator
            ...
            // The thing you can improve upon
            i++;
            char version[9];
            for(int j = 0;line[i+j]!='\0' && j < 8; j++)
            {
                version[j] = line[i+j];   
            }
            version[8] = '\0';
            ...
            // TODO: extract query from request-target eg /cat.html
            char *absstart = strchr(line, '/');
            strtok(line, " ");
            strtok(NULL, " ");
            char *stop = strtok(NULL, "\0");
            char *absstop;
            if(strchr(line, '?') != NULL)
                absstop = strchr(line, '?');
            else
                 absstop = stop;
            int pathlength = absstop - absstart;
            char absolute[pathlength + 1];
            for(int l = 0; l < pathlength; l++)
            {
                absolute[l] = line[4 + l];
            }
            absolute[pathlength] = '\0';
            char * question = strchr(line, '?');
            int qindex = question - line;
            int qlength = stop - question;
            char query[qlength + 1];
            if(question != NULL)
            {
                for(int i = 0; line[qindex + i]!= ' '; i++)
                {
                    query[i] = line[qindex + i];
                }
                query[qlength] = '\0';
            }
            ...
            char path[strlen(root) + pathlength + 1];
            strcpy(path, root);
            strcat(path, absolute);
            ...
            char *xstart = strchr(absolute, '.');
            char *xstop = strchr(absolute, '\0');
            int xlength = xstop - xstart;
            char extension[xlength];
            int xindex = xstart - absolute;
            for(int i = 0; i < xlength; i++)
            {
                extension[i] = absolute[xindex + i];
            }
            extension[xlength] = '\0';
            ...
            if (strcasecmp("php", extension) == 0)
            {
                ...
                file = popen(command, "r");//popen? program open?
                if (file == NULL)
                {
                    error(500);
                    continue;
                ...
                // subtract php-cgi's headers from response body's size to get content's length
                haystack = body;
                /* size is load file size*/
                needle = memmem(haystack, size, "\r\n\r\n", 4);//memmem? like strstr
                if (needle == NULL)
                {
                    error(500);
                    continue;
                }//we need to leave a line after the DOCTYPE html in the php file
            ...
            // announce OK only if connected
            printf("\033[32m"); // what is this?
